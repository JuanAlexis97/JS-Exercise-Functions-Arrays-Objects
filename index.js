...
makePersonObject(10, "Jess", "jess@jess.com");
/**
 * ### Challenge `getName`
 * 
 * @instructions
...
function getCarInfoById(arr, id) {
  const carInfoById = arr[arr.id]
  return `This is a ${carInfoById(id).car_make} ${carInfoById[id].car_model}`;
}
...
function sortCarInventory(arr) {
  return arr.sort(function(a,b){
    const aCar = a.car_model.toUpperCase();
    const bCar = b.car_model.toUpperCase();
    if(aCar < bCar){
      return -1;
    }
    else if(aCar > bCar){
...
*/
function getModelYears(carYears) {
  const carYearsList = carYears[];
  for(var i = 0; i < carYears.length; i++){
    carYearsList.push(carYears.length[].car_year);
  }
  return carYearsList;
}
...
function getOlderCars(arr, maxYear) {
    const listOfOldCars = arr[arr.car_year];
    return listOfOldCars.slice(0, maxYear);
...
function getGermanCars(arr) {
  const germanCars = arr[];
  for(var i = 0; i < arr.length; i++){
    if(arr.car_make === "Audi" || arr.car_make === "BMW" || arr.car_make === "Mercedes-Benz"){
      germanCars.car_make.push();
    }
    return germanCars;
  }
}
